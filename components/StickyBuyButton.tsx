...
export default function StickyBuyButton() {
  ...
  const handleComprar = () => { addItem({ id: uuidv4(), name: "Antes que eu entendesse — Kelly Marques", price: t.price, quantity: 1 }); trackEvent("add_to_cart", "/", { source: "sticky" }); router.push(`/${locale}/carrinho`) }
  if (!visible) return null
  return (<div className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-white/95 backdrop-blur-md border-t shadow-2xl px-4 py-3 safe-bottom"><button onClick={handleComprar} className="w-full bg-gradient-to-r from-rosa-600 to-rosa-700 text-white font-bold py-3.5 rounded-full text-sm uppercase tracking-wide shadow-lg">🛒 {t.currency === "USD" ? "$" : "R$"} {t.price.toFixed(2)}</button></div>)
}
