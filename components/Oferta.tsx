...
export default function Oferta() {
  ...
  const handleComprar = () => { addItem({ id: uuidv4(), name: "Antes que eu entendesse — Kelly Marques", price: 119.0, quantity: 1 }); trackEvent("add_to_cart", "/", { source: "oferta" }); router.push(`/${locale}/carrinho`) }
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8 items-center">
          ...
          <div className="bg-white/70 backdrop-blur-md border border-white/20 rounded-2xl shadow-xl p-5 sm:p-6 md:p-8">
            ...
            <div className="text-center"><p className="text-gray-500 text-xs sm:text-sm mb-1">{t.oferta.from} <span className="line-through">R$ 149,00</span> {t.oferta.priceLabel}</p><span className="text-4xl sm:text-5xl md:text-6xl font-playfair font-bold text-rosa-700">R$ 119</span><p className="text-gray-500 text-xs sm:text-sm mb-4 mt-1">{t.oferta.installments}</p><ShippingCalculator /><button onClick={handleComprar} className="w-full mt-4 bg-gradient-to-r from-rosa-600 to-rosa-700 hover:from-rosa-700 hover:to-rosa-800 text-white font-bold py-3 sm:py-4 rounded-full text-sm sm:text-lg shadow-2xl uppercase tracking-wider">{t.oferta.cta}</button></div>
          </div>
        </div>
      </div>
    </section>
